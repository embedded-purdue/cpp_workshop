...
// Physical constants
static constexpr float SPEED_OF_SOUND_CM_PER_US = 0.0343f;  // cm/μs
...
UltrasonicSensor::UltrasonicSensor(gpio_num_t trigger_pin, 
                                   gpio_num_t echo_pin, 
                                   uint32_t timeout_us)
...
UltrasonicSensor::Status UltrasonicSensor::measure_distance(float& distance) {
...
UltrasonicSensor::Status UltrasonicSensor::measure_distance_avg(float& distance, 
                                                                uint8_t samples) {
    if (samples == 0) {
        return Status::Error;
    }
...
bool UltrasonicSensor::is_object_in_range(float threshold_distance) {
...
void UltrasonicSensor::set_timeout(uint32_t timeout_us) {
    timeout_us_ = timeout_us;
}

uint32_t UltrasonicSensor::get_timeout() const {
    return timeout_us_;
}

void UltrasonicSensor::send_trigger_pulse() {
    // Ensure trigger is low
    trigger_gpio_.set_low();
    esp_rom_delay_us(TRIGGER_SETTLE_US);
    
    // Send 10μs high pulse
    trigger_gpio_.set_high();
    esp_rom_delay_us(TRIGGER_PULSE_US);
    trigger_gpio_.set_low();
}

UltrasonicSensor::Status UltrasonicSensor::measure_echo_pulse(uint32_t& pulse_duration_us) {
...
float UltrasonicSensor::pulse_to_distance(uint32_t pulse_duration_us) {
